# Homepage Condensed — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace `src/app/page.tsx` with a version that removes all opaque section backgrounds so the ambient video runs continuously through the page, and tightens vertical padding throughout.

**Architecture:** Single-file change to `src/app/page.tsx`. All four sections get `bg-transparent`. The hero's full-bleed product image moves to a contained right-column portrait card. Light-section components (`PaperBackground`, `HandDrawnDivider`) are removed and text colors shift to dark-scheme values. No new components are created.
...
**Imports removed:** `PaperBackground`, `HandDrawnDivider` (no longer used)

**Imports kept:** `InkUnderline`, `SketchFrame`, `ProductCard`, `getAllProducts`, `getCollectionProducts`, `mapShopifyProductToHanaBiProduct`, fallback product arrays, `Image`, `Link`

---

## Task 1: Replace `src/app/page.tsx`
...
- Video plays continuously through all four sections — no opaque background breaks
- Hero: text left, garment portrait card right (dark border, name + HB-001 overlaid bottom-left)
- No full-bleed hero image behind the text
- "What is Hana-Bi?" and "Archive Strip" use light cream text over the dark video
- Product cards and SketchFrame borders use dark-border tokens
- Sections feel tighter (less vertical space between them)

- [ ] **Step 4: Commit**

```bash
cd c:\hana-bi && git add src/app/page.tsx && git commit -m "feat: homepage — continuous video background, condensed sections, hero portrait card"
```
...
**Spec coverage:**
- ✅ `bg-transparent` on all sections — hero, What is Hana-Bi?, Current Drop, Archive Strip
- ✅ `grain` removed from hero section
- ✅ Full-bleed hero image removed
- ✅ Hero product image as contained `aspect-[3/4]` portrait card in right column
- ✅ Catalog number (HB-001) + garment name overlaid at `absolute bottom-4 left-4` inside card
- ✅ Story paragraph and "View Piece" link removed from hero card
- ✅ `PaperBackground` removed and import dropped
- ✅ `HandDrawnDivider` removed and import dropped
- ✅ `py-24` → `py-16` on What is Hana-Bi?, Current Drop, Archive Strip
- ✅ Eyebrow text in formerly-light sections: `color: "var(--hb-sienna)"`, mono font
- ✅ `h3`/`h4` headings in formerly-light sections: `text-[#faf8f4]`
- ✅ Body text in formerly-light sections: `text-[var(--hb-dark-muted)]`
- ✅ Archive strip borders: `border-[var(--hb-dark-border)]`
- ✅ `InkUnderline` kept on both headings
- ✅ `SketchFrame` kept, `strokeOpacity={0.3}` unchanged

**Placeholder scan:** None. All code is complete and explicit.

**Type consistency:** All component props match their existing signatures. No new types introduced.
